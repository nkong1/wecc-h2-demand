...
missing_combustion_data_folder = (
    base_path / "inputs" / "facilities_missing_combustion_data"
)

# Fuel use projections taken from the EIA Energy Outlook 2050 Reference Case Table 2:
fuel_use_projection_path = (
    base_path / "inputs" / "eia_aeo_industrial_fuel_use_projections.csv"
)

# CO2 emissions breakdown by sector (from DOE Pathways to Commercial Liftoff: Industrial Decarbonization Fig 2a.2)
co2_emissions_breakdown_path = (
    base_path / "inputs" / "doe_co2_emissions_breakdown_by_industry.csv"
)
...
# Final output path
demand_by_lz_path = (
    base_path.parent / "outputs" / "industry" / "demand_by_load_zone.csv"
)
...
sector_by_naics = {
    ...
    "Chemicals": [325],
    "Refineries": [324110],
    "Glass": [327211, 327212, 327213, 327215],
}
...
def get_high_heat_emissions_share(sector):
    ...
    total_combustion = (
        sector_row[
            ["low_temp_heat", "mid_temp_heat", "high_temp_heat", "on_site_power"]
        ]
        .sum(axis=1)
        .iloc[0]
    )
...
def calc_discrepancies(results_by_facility_df):
    ...
    west_breakdown_by_fuel_df = results_by_facility_df[
        results_by_facility_df["inWestCensus"] == True
    ].copy()

    west_breakdown_by_fuel_df["NAICS Code"] = (
        west_breakdown_by_fuel_df["NAICS Code"].astype(str).str.split(".").str[0]
    )

    west_breakdown_by_fuel_df_grouped = west_breakdown_by_fuel_df.groupby("NAICS Code")
    mecs_data = pd.read_csv(
        mecs_data_path, index_col="NAICS Code", dtype={"NAICS Code": str}
    )

    mecs_data = (
        mecs_data.replace("*", 0.25)  # turn '*' into 0.25
        .dropna(subset=["Sector Category"])  # remove rows missing Sector Category
        .apply(
            pd.to_numeric, errors="coerce"
        )  # convert all numeric cols to numbers
    )

    mecs_data["total_fossil_mmbtu"] = (
        mecs_data["Residual Fuel Oil"]
        + mecs_data["Distillate Fuel Oil"]
        + mecs_data["Natural Gas"]
        + mecs_data["HGL (excluding natural gasoline)"]
        + mecs_data["Coal"]
        + mecs_data["Coke and Breeze"]
    ) * 1e6
    ...
    for naics in all_naics:
        ...
        discrepancy_list.append(
            {
                "NAICS Code": naics,
                "Sector": get_sector(naics),
                "mecs_mmbtu": mecs_fuel_total_mmbtu,
                "ghgrp_mmbtu": ghgrp_fuel_total_mmbtu,
                "discrepancy_mmbtu": discrepancy_mmbtu,
                "mecs_to_ghgrp_ratio": mecs_to_ghgrp_ratio,
            }
        )
    ...
    sector_discrepancy_df = (
        naics_discrepancy_df.groupby("Sector")
        .agg(
    ...
    sector_discrepancy_df["mecs_to_ghgrp_ratio"] = (
        sector_discrepancy_df["mecs_mmbtu"] / sector_discrepancy_df["ghgrp_mmbtu"]
    )

    # Sort by largest discrepancy
    sector_discrepancy_df = sector_discrepancy_df.sort_values(
        "discrepancy_mmbtu", ascending=False
    )
...
def project_sector_consumption(sector, fuel_use, year):
    ...
    mecs_sector_row = (
        mecs_fuel_data.drop(
            columns=["EIA Sector:", "NAICS Code", "Total", "Net Electricity", "Other"]
        )
    ...
    emissions_factors_df = (
        emissions_factors_df[~emissions_factors_df["EIA MECS Category"].isna()]
        .groupby("EIA MECS Category")
        .first()
        .reset_index()
    )
    ...
    fuel_use_by_category_filtered = fuel_use_projections_df[
        fuel_use_projections_df["Year"].isin([2022, year])
    ...
    fuel_growth_by_category_dict = {
        col: (
            fuel_use_by_category_filtered[col].iloc[1]
            - fuel_use_by_category_filtered[col].iloc[0]
        )
        / fuel_use_by_category_filtered[col].iloc[0]
        for col in fuel_use_by_category_filtered.columns
        if col != "Year"
    }
    ...
    for mecs_fuel_type in sector_fuel_consumption.index:
        ...
        if fuel_use_mmbtu != 0:
            ...
            projected_fuel_use += fuel_use_mmbtu * (
                1 + fuel_growth_by_category_dict[aeo_fuel_type]
            )
...
def model_one_year(high_temp_decarb_by_sector, year):
    ...
    fuel_use_by_category_filtered = fuel_use_by_category_df[
        fuel_use_by_category_df["Year"].isin([2022, year])
    ].reset_index(drop=True)
    ...
    results_by_facility, breakdown_by_fuel = calc_epa_ghgrp_fuel_consumption(
        high_temp_decarb_by_sector, fuel_growth_by_category_dict
    )
    ...
    breakdown_by_fuel_df.to_csv(
        logs_path / f"{year}_unadjusted_demand_by_unit_fuel.csv", index=False
    )
    results_by_facility_df.to_csv(
        logs_path / f"{year}_unadjusted_demand_by_facility.csv", index=False
    )
    ...
    wecc_results_by_facility = results_by_facility_df[
        results_by_facility_df["inWECC"] == True
    ]

    average_facility_proj_demand_by_sector = wecc_results_by_facility.groupby("Sector")[
        "proj_fuel_demand_mmBtu"
    ].mean()
    average_facility_demand_by_sector = wecc_results_by_facility.groupby("Sector")[
        "fuel_demand_mmBtu"
    ].mean()
    ...
    for file_path in missing_combustion_data_folder.glob("*csv"):
        ...
        missing_facilities_df["proj_fuel_demand_mmBtu"] = (
            average_facility_proj_demand_by_sector[sector]
        )
        missing_facilities_df["fuel_demand_mmBtu"] = average_facility_demand_by_sector[
            sector
        ]

        # Format to match columns in the results_by_facility_df
        missing_facilities_df = missing_facilities_df[
            [
                "Facility Id",
                "Facility Name",
                "Primary NAICS Code",
        ...
        ].rename(columns={"Primary NAICS Code": "NAICS Code"})

        # Add results to the running list
        results_by_facility_df = pd.concat(
            [results_by_facility_df, missing_facilities_df]
        )
    ...
    for sector in sector_by_naics.keys():
        sector_row = discrepancies_by_sector[
            discrepancies_by_sector["Sector"] == sector
        ].iloc[0]
        ...
        if discrepancy_mmbtu < 0:
            ...
            results_by_facility_df.loc[
                results_by_facility_df["Sector"] == sector, "fuel_demand_mmBtu"
            ] *= mecs_to_ghgrp_ratio
            results_by_facility_df.loc[
                results_by_facility_df["Sector"] == sector, "proj_fuel_demand_mmBtu"
            ] *= mecs_to_ghgrp_ratio
        ...
        # If our estimates are low, we adjust by disaggregating "unaccounted-for demand" across non-GHGRP facilities in the West Census
            ...
            extra_facilities_path = base_path / "inputs" /  "extra_epa_frs_facilities_west" / f"{sector}_facilities.csv"
            extra_facilities_df = pd.read_csv(extra_facilities_path)

            if extra_facilities_df.empty:
                continue

            print(extra_facilities_df.head())
            # Fill in the fuel demand
            extra_facilities_df["fuel_demand_mmBtu"] = discrepancy_mmbtu / len(extra_facilities_df.reset_index(drop=True))
            print(discrepancy_mmbtu)
            print(len(extra_facilities_df.reset_index(drop=True)))
            ...
            extra_facilities_df["proj_fuel_demand_mmBtu"] = extra_facilities_df[
                "fuel_demand_mmBtu"
            ].apply(
                lambda x: cached_project_sector_consumption(sector, x, year)
                * high_temp_decarb_pct
                / 100
            ...
            # Format to match columns in the results_by_facility_df
            extra_facilities_df = extra_facilities_df[
                [
                    "registry_id",
                    "primary_name",
                    "naics_code",
            ...
            # Add results to the running list
            results_by_facility_df = pd.concat(
                [results_by_facility_df, extra_facilities_df]
            )
    ...
    results_by_facility_df.to_csv(
        logs_path / f"{year}_west_census_and_wecc_final_demand_by_facility.csv",
        index=False,
    )

    # Filter the facilities to only those within WECC bundaries
    filtered_df = results_by_facility_df[
        results_by_facility_df["inWECC"] == True
    ].copy()
    ...
    grid_output_path = (
        ...
        / "outputs"
        / "industry"
        / f"{year}_wecc_h2_demand_5km_resolution.gpkg"
    )
    ...
    filtered_df.to_csv(
        logs_path / f"{year}_final_industry_demand_by_facility.csv", index=False
    )
...
def calc_epa_ghgrp_fuel_consumption(
    high_temp_pct_decarb_by_sector: list, fuel_growth_by_category_dict: dict
):
    """
    ...
    fuel_emissions_dict = fuel_emissions_df.set_index("Fuel Type")[
        "kg CO2 per mmBtu"
    ].to_dict()

    # Create a dictionary mapping each fuel type to a broader fuel category for which we have fuel consumption projections
    aeo_fuel_category_dict = fuel_emissions_df.set_index("Fuel Type")[
        "EIA AEO Category"
    ].to_dict()
    ...
    for file_name in os.listdir(units_and_fuel_folder):
        ...
        results_by_sector = []
        industry_facilities_df = pd.read_csv(
            base_path / units_and_fuel_folder / file_name
        )
        ...
        for facility_id, facility_df in sector_facilities_grouped:
            ...
            naics = get_naics_code(
                int(industry_facilities_df.iloc[0]["Primary NAICS Code_y"])
            )
            inWECC = facility_df["inWECC"].iloc[0]
            inWestCensus = facility_df["inWestCensus"].iloc[0]

            # Create running totals for fuel demand
            ...
            for unit_name, unit_df in facility_units_grouped:
                ...
                unit_df["Specific Fuel Type"] = unit_df["Specific Fuel Type"].fillna(
                    "Natural Gas"
                )

                # Retreive the unit's total CO2 emissions
                unit_CO2_emissions_mt = unit_df.iloc[0][
                    "Unit CO2 emissions (non-biogenic)"
                ]

                # If there are multiple fuels, find the average emissions factor
                ...
                for _, fuel_df in unit_df.iterrows():
                    ...
                    # Set consumes_biofuels to True if a biofuel is consumed by the unit
                    if "Biofuels" in aeo_fuel_category_dict[fuel]:
                        consumes_biofuels = True

                    # Get the emissions factor for the fuel
                    unit_fuels.append(fuel)
                    emissions_factor = fuel_emissions_dict[fuel]
                    emissions_factor_total += emissions_factor

                # Use the average emissions factor across all fuels to estimate the fuel demand for the unit
                if emissions_factor_total == 0:
                    continue

                avg_emissions_factor = emissions_factor_total / len(
                    unit_fuels
                )  # emissions factor is in metric tons

                high_temp_decarb_pct = high_temp_pct_decarb_by_sector[
                    list(sector_by_naics.keys()).index(sector_name)
                ]

                high_temp_decarb_factor = high_temp_decarb_pct / 100

                unit_demand_mmBtu = (
                    unit_CO2_emissions_mt * 1000 / avg_emissions_factor
                )  # multiplying by 1000 to convert from mt to kg

                # Remove the inclusion of biofuels in the fuel demand
                if consumes_biofuels:
                    unit_demand_mmBtu *= (len(unit_fuels) - 1) / len(unit_fuels)
                    unit_CO2_emissions_mt *= (len(unit_fuels) - 1) / len(unit_fuels)
                ...
                for fuel in unit_fuels:
                    ...
                    # Project fuel use using EIA AEO 25 projections
                    aeo_fuel_category = aeo_fuel_category_dict[fuel]
                    projected_fuel_growth = fuel_growth_by_category_dict[
                        aeo_fuel_category
                    ]

                    projected_fuel_demand_mmbtu = (
                        (1 + projected_fuel_growth)
                        * fuel_demand_by_unit_fuel
                        * high_temp_decarb_factor
                        * get_high_heat_emissions_share(sector_name)
                    )

                    breakdown_by_fuel.append(
                        {
                            ...
                            "NAICS Code": naics,
                            "Sector": get_sector(naics),
                            "unit_CO2_emissions": unit_CO2_emissions_mt,
                            "CO2_Emissions": CO2_emissions_by_unit_fuel,
                            "fuel_growth_proportion": projected_fuel_growth,
                            "average EF": avg_emissions_factor,
                            "high_temp_decarb_factor": high_temp_decarb_factor,
                            "high_temp_emissions_share": get_high_heat_emissions_share(
                                sector_name
                            ),
                            "fuel_demand_mmBtu": fuel_demand_by_unit_fuel,
                            "proj_fuel_demand_mmBtu": projected_fuel_demand_mmbtu,
                            "avg_emissions_factor": avg_emissions_factor,
                            "inWestCensus": fuel_df["inWestCensus"],
                            "inWECC": inWECC,
                        }
                    )

                    # Add the fuel demand to the facility's running total for fuel demand, scaled by scale_demand
            ...
            results_by_sector.append(
                {
                    ...
                    "Latitude": latitude,
                    "Longitude": longitude,
                    "fuel_demand_mmBtu": facility_fuel_demand_mmBtu,
                    "proj_fuel_demand_mmBtu": proj_facility_fuel_demand_mmBtu,
                    "inWestCensus": inWestCensus,
                    "inWECC": inWECC,
                }
            )
        ...
        all_results_by_facility = pd.concat(
            [all_results_by_facility, industry_results_df]
        )
...
def model_industry_demand(high_temp_pct_decarbonization, years):
    ...
    load_zone_summary = load_zone_summary.sort_values(
        by=["load_zone", "year"]
    ).reset_index(drop=True)
